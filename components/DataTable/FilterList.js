...
const getDataCounter = (data) => {
    ...
    data.map((r) => {
        ...
        primarySites.push(...r.primary_sites)
        diseaseTypes.push(...r.disease_types)
        experimentalStrategies.push(...r.experimental_strategies)
    })

    return {
        'program': new Counter(programs),
        'primary site': new Counter(primarySites),
        'disease type': new Counter(diseaseTypes),
        'experimental strategy': new Counter(experimentalStrategies)
    }
}
...
const FilterList = ({data, updateDataFn}) => {
    ...
    const filterData = useCallback((data, filter) => {
        ...
        return data.filter((r) => {
            ...
            const passFilterPrimarySites =
                (filter['primary site'].length > 0) ?
                    filter['primary site'].filter(primarySite => r.primary_sites.includes(primarySite)).length > 0 : true;
            const passFilterDiseaseTypes =
                (filter['disease type'].length > 0) ?
                    filter['disease type'].filter(diseaseType => r.disease_types.includes(diseaseType)).length > 0 : true;
            const passFilterExperimentalStrategies =
                (filter['experimental strategy'].length > 0) ?
                    filter['experimental strategy'].filter(experimentalStrategy => r.experimental_strategies.includes(experimentalStrategy)).length > 0 : true;

            return (passFilterProgram && passFilterPrimarySites && passFilterDiseaseTypes
                && passFilterExperimentalStrategies)
        })
    }, [])
